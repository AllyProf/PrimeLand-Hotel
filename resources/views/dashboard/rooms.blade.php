...
<div class="app-title">
  <div>
    ...
    <p>{{ isset($room) ? 'Edit Room' : 'Add New Room to PrimeLand Hotel' }}</p>
  </div>
    ...
    <div class="tile">
      ...
      <form id="roomForm" method="POST" action="{{ isset($room) ? route('admin.rooms.update', $room) : route('admin.rooms.store') }}" enctype="multipart/form-data">
        ...
        <div class="wizard-step" data-step="2" style="display: none;">
          <h4 class="mb-4"><i class="fa fa-dollar"></i> Pricing Information</h4>
        ...
        <div class="wizard-step" data-step="3" style="display: none;">
          ...
          <div class="form-group">
            ...
            <input class="form-control" type="text" id="wifi_password" name="wifi_password" placeholder="e.g., PrimeLand2024" maxlength="255">
...
let currentStep = 1;
const totalSteps = 5;
// Get exchange rate from backend (fetched from API)
let exchangeRate = {{ $exchangeRate ?? 2400 }};
...
function convertToTZS(usdAmount) {
  if (!usdAmount || isNaN(usdAmount) || usdAmount <= 0) {
    return '';
  }
  const tzsAmount = (parseFloat(usdAmount) * exchangeRate).toLocaleString('en-US', {
    minimumFractionDigits: 0,
    maximumFractionDigits: 0
  });
  return `≈ ${tzsAmount} TZS`;
}
...
document.getElementById('roomForm').addEventListener('submit', function(e) {
  ...
  swal({
    ...
    if (isConfirm) {
      ...
      loadingSpinner.innerHTML = '<div style="background: white; padding: 30px; border-radius: 8px; text-align: center;"><i class="fa fa-spinner fa-spin fa-3x" style="color: #e77a3a;"></i><p style="margin-top: 15px; font-size: 16px;">Saving room data...</p></div>';
...
// Exchange rate is already loaded from backend (fetched from API via CurrencyExchangeService)
console.log('Exchange rate loaded from API: 1 USD =', exchangeRate.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}), 'TZS');
</script>
@endsection
