      ...
      <form id="changeTypeForm" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="form-group">
                <label for="new_price_per_night">Price per Night (USD) <span class="text-danger">*</span></label>
                <div class="input-group">
                  <div class="input-group-prepend">
                    <span class="input-group-text">$</span>
                  </div>
                  <input class="form-control" type="number" id="new_price_per_night" name="price_per_night" step="0.01" min="0" required>
                </div>
                <small class="form-text text-muted" id="price_tzs_display"></small>
              </div>
            </div>
          </div>
...
function executeBulkAction(action, value) {
  ...
  Swal.fire({
    ...
    confirmButtonColor: action === 'delete' ? "#d33" : "#e77a3a",
...
function updatePriceDisplay(price) {
  const exchangeRate = {{ $exchangeRate ?? 2400 }};
  const tzsPrice = (parseFloat(price) * exchangeRate).toFixed(0);
  document.getElementById('price_tzs_display').textContent = `≈ ${tzsPrice} TZS`;
}
