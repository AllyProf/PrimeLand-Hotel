        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              ...
              <div id="recommended_price_usd" style="display: none; margin-top: 5px;">
                <small class="text-muted">≈ $<span id="recommended_price_usd_value">0</span> USD</small>
              </div>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group">
              ...
              <div class="input-group">
                ...
                <input class="form-control" type="number" id="total_price" name="total_price" step="0.01" min="0" placeholder="0.00" required>
              </div>
              <small class="form-text text-muted">Enter total price (can be lower than recommended for discounts)</small>
              <div id="total_price_tzs" style="display: none; margin-top: 5px;">
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              ...
              <div class="input-group">
                <div class="input-group-prepend">
                  <span class="input-group-text" id="amount_paid_currency">$</span>
                </div>
                <input class="form-control" type="number" id="amount_paid" name="amount_paid" step="0.01" min="0" placeholder="0.00" required>
              </div>
              <small class="form-text text-muted">Enter the amount paid</small>
              <div id="amount_paid_tzs" style="display: none; margin-top: 5px;">
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              ...
              <div class="input-group">
                <div class="input-group-prepend">
                  <span class="input-group-text" id="remaining_amount_currency">$</span>
                </div>
              ...
              <div id="remaining_amount_tzs" style="display: none; margin-top: 5px;">
          ...
          <div class="review-summary">
            ...
            <div class="card mb-3">
              ...
              <div class="card-body">
                <table class="table table-sm mb-0">
                  <tr>
                    <td width="40%"><strong>Exchange Rate:</strong></td>
                    <td id="review_exchange_rate">-</td>
                  </tr>
                  <tr>
...
function validateWizardStep(step) {
  if (step === 1) {
    ...
    if (!guestType) {
      swal({ title: "Validation Error", text: "Please select guest type", type: "error", confirmButtonColor: "#e77a3a" });
      return false;
    }
    ...
    if (guestType === 'international') {
      ...
      if (!nationality || nationality === '') {
        swal({ title: "Validation Error", text: "Please select nationality", type: "error", confirmButtonColor: "#e77a3a" });
        return false;
      }
    }
    
    if (!fullName) {
      swal({ title: "Validation Error", text: "Please enter full name", type: "error", confirmButtonColor: "#e77a3a" });
      return false;
    }
    
    if (!email || !email.includes('@')) {
      swal({ title: "Validation Error", text: "Please enter a valid email address", type: "error", confirmButtonColor: "#e77a3a" });
      return false;
    }
    
    if (!phone) {
      swal({ title: "Validation Error", text: "Please enter phone number", type: "error", confirmButtonColor: "#e77a3a" });
      return false;
    }
    
    if (!guests || parseInt(guests) < 1) {
      swal({ title: "Validation Error", text: "Please enter number of guests", type: "error", confirmButtonColor: "#e77a3a" });
      return false;
    }
    ...
    if (!roomType) {
      swal({ title: "Validation Error", text: "Please select room type", type: "error", confirmButtonColor: "#e77a3a" });
      return false;
    }
    
    if (!checkIn) {
      swal({ title: "Validation Error", text: "Please select check-in date", type: "error", confirmButtonColor: "#e77a3a" });
      return false;
    }
    
    if (!checkOut) {
      swal({ title: "Validation Error", text: "Please select check-out date", type: "error", confirmButtonColor: "#e77a3a" });
      return false;
    }
    
    if (!roomId || roomId === '') {
      swal({ title: "Validation Error", text: "Please select an available room", type: "error", confirmButtonColor: "#e77a3a" });
      return false;
    }
    ...
    if (!totalPrice || parseFloat(totalPrice) <= 0) {
      swal({ title: "Validation Error", text: "Please enter total price", type: "error", confirmButtonColor: "#e77a3a" });
      return false;
    }
    
    if (!paymentMethod) {
      swal({ title: "Validation Error", text: "Please select payment method", type: "error", confirmButtonColor: "#e77a3a" });
      return false;
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const updateCurrencyDisplay = function() {
    ...
    // For International: Show TZS conversion below, hide USD conversion
    ...
    tzsElements.forEach(id => {
      const el = document.getElementById(id);
      if (el) el.style.display = isTanzanian ? 'none' : 'block';
    });
    
    usdElements.forEach(id => {
      const el = document.getElementById(id);
      if (el) el.style.display = isTanzanian ? 'block' : 'none';
    });
  ...
  function toggleNationalityField() {
    ...
    if (guestTypeHint) {
      if (isTanzanian) {
        ...
        guestTypeHint.textContent = 'Prices will be displayed in USD with TZS conversion';
        guestTypeHint.style.color = '#666';
      }
    }
  ...
  function recalculateOnGuestTypeChange() {
    ...
    if (previousValue && previousValue !== guestTypeSelect.value) {
      ...
      if (newTotalPrice > 0 && newAmountPaid > 0) {
        const remaining = newTotalPrice - newAmountPaid;
        remainingAmountInput.value = remaining >= 0 ? remaining.toFixed(2) : '0.00';
      } else if (newTotalPrice > 0) {
        remainingAmountInput.value = newTotalPrice.toFixed(2);
      } else {
  ...
  function calculatePaymentFromAmount(skipAutoFill = false) {
    // Use total price from input field (can be different from recommended for discounts)
    const totalPrice = parseFloat(totalPriceInput.value) || 0;
    const amountPaid = parseFloat(amountPaidInput.value) || 0;

    // Only auto-fill total price if skipAutoFill is false (i.e., when recommended price changes, not when user edits)
